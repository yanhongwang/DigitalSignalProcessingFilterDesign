...
protodata( GtkButton *button, gint mode )
{
   ...
   glong raw_data[ MAX ],ripe_data[ MAX ];
...
*/
   ...
   if( ( system( sys_command ) ) == -1 )
      gtk_statusbar_push( GTK_STATUSBAR( statusbar ), 1
	 , "當在執行 gnuplot 的時候，有錯誤發生，請自行解決 gnuplot 的問題後，自執行頻譜的繪製" );
   else
      gtk_statusbar_push( GTK_STATUSBAR( statusbar ), 1
	 , spectrum_message );
...
Transfer_function( GtkButton *button, gint mode )
{
   ...
   gchar *sift = ",; ", *segment = "", substitute[ MAX ];

   // term 代表項數，也就是幾階的意思，也是係數的數目啦！
   gint term_a = 0, term_b = 0;

   // 可以放置係數的陣列
   gfloat a_factor[ MAX ], b_factor[ MAX ];

   gint i = 0, j = 0;
//////////////////////////////
float data_xn[ MAX ] = { 10, 14, 16, 15, 11, 14, 12 };
